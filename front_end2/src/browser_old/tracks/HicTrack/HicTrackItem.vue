<template>
  ...
  <div v-if="isVisible">
    ...
    <chrom-view class="absolute -rotate-90 origin-left -bottom-20 -left-20 z-100000000" ref="secondChromView" :chrom="corenavStore.chrom" :chromBands="chromBands" :chromNames="chromNames" :chromSizes="chromSizes"
      :end="corenavStore.end" :isSelectorVisible="true" :labelWidth="0" :max="corenavStore.max" :min="corenavStore.min"
      :start="corenavStore.start" type="full" @zoomTo="zoomTo">ddd
    </chrom-view>
  </template> -->
    <div ref="canvasContainer" class="basic-canvas" :style="props.style">
...
import { defineComponent, ref, onMounted, reactive, watch, computed, defineProps, defineEmits } from 'vue';
import NavBar from "../NavBar.vue";
import ChromView from "../ChromView.vue";
...
const secondChromView = ref(null)
var moveLeftPixels = 500; // Replace 50 with the actual number you want

// Function to apply CSS transformations
function moveAndRotateDiv(div) {
  // Apply a 90-degree rotation around the right end
  ...
  div.style.zIndex = '10';
  // Move the div down by 45px and to the left by the specified number of pixels
  // div.style.top = '45px';
  // div.style.left = -moveLeftPixels + 'px';
}
...
onMounted(async () => {
  ...
  () => chrom1.value, () => start1.value, () => end1.value], (newStart, newEnd) => {
    if (type.value == 'square') {
      ...
      showSpin.value = true
      plotHic(straw, chrom.value, start.value, end.value, chrom1.value, start1.value, end1.value, width.value, height.value, bpResolutions[zoomIndex])
      showSpin.value = false
      // moveAndRotateDiv(secondChromView.value)
      // secondChromView.value.style.position = 'absolute';
      // secondChromView.value.style.top = '0';
      // secondChromView.value.style.right = '0';
  // div.style.width = '100px';
  // div.style.height = '100px';
  // div.style.backgroundColor = 'red';
  // div.style.zIndex = '10';
  ...
  //     canvasContainer.value.innerHTML = ''
  //     showSpin.value = true
  //     plotHic(straw, chrom.value, start.value, end.value, width.value, height.value, bpResolutions[zoomIndex])
  //     showSpin.value = false
  //     // showSpin.value = false;
  // }, { deep: true });
