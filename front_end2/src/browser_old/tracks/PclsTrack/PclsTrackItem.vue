    ...
    watch([() => option.value.series, () => chrom.value, () => start.value, () => end.value], async (newStart, newEnd) => {
        if (end.value - start.value < visibilityWidth) {
            ...
            canvas.value.addEventListener('click', function (event) {
                ...
                // Check each item to see if the click occurred within its bounding box
                for (let i = 0; i < lines.length; i++) {
                    const item = lines[i];
                    const bbox = item._boundingBox;
                    if (x > bbox.x && x < bbox.x + bbox.width && y > bbox.y && y < bbox.y + bbox.height) {
                        // Click is inside this item's bounding box, so draw the text
                        const ctx = canvas.value.getContext('2d');
                        ctx.save();
                        ctx.fillStyle = 'black'; // Or any color you want for the text
                        ctx.font = '10px sans-serif'; // Or any font style you want
                        ctx.fillText(item._text, bbox.x, 6 * h / 8 + 6); // You may need to adjust this position
                        ctx.restore();
                        break; // Stop checking other items since we found the clicked one
                    }
                }
            });
        } else {
