...
const _drawItem = (canvas, x, y, w, h, gene, colors, c, quick) => {
    ...
    ctx.fillStyle = _getColor(colors, "text", gene) || props.option.series[0].label.color;
...
onMounted(() => {
    ...
    watch([()=>option.value.series,() => chrom.value, () => start.value, () => end.value], async (newStart, newEnd) => {
        ...
        //     fetchGeneAnnoFileData(url).then(d => {
        //         console.group(d)
        //         d = processGeneList(d)
    ...
    // watch([() => chrom.value, () => start.value, () => end.value], (newStart, newEnd) => {
    //     showSpin.value = true
    //     if (end.value - start.value < visibilityWidth) {
    //         isVisible.value = true
    //         canvasReady.value = false
