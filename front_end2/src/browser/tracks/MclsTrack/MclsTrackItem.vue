...
import { drawPlotX } from '../../hooks/BaseRowTrack2'
...
onMounted(() => {
    watch([()=>option.value.series,()=>chrom.value,() => start.value, () => end.value], async (newStart, newEnd) => {
        ...
        drawPlotX(canvas.value, lines, false, canvasContainer.value, h.value, prepad.value, postpad.value, showLabel.value, _drawItemBed, _drawItemBedpe, _measureWidth, start.value, end.value)
        showSpin.value = false
        } else {
