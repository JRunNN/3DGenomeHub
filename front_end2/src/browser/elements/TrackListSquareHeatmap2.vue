  ...
  <div v-if="isLoaded" :id="props.sessionId" class="track-list flex flex-col pl-2 pr-2 w-full">
    ...
    <div id="screenshot-holder">
      <div class="flex">
        ...
        <div class="grow">
          <chrom-view-h :chrom="corenavStore.chrom" :chromBands="chromBands" :chromNames="chromNames"
            :chromSizes="chromSizes" :end="corenavStore.end" :isSelectorVisible="true" :labelWidth="0"
            :max="corenavStore.max" :min="corenavStore.min" :start="corenavStore.start" type="full" @zoomTo="zoomTo">
          </chrom-view-h>
        </div>
      </div>

      <div ref="trackholderRef" class=" absolute inset-0 track-holder " style="position: relative">
        <!-- All the other components -->
        <template v-for="(trackComponent, index) in trackComponents">
          <TrackItemSquareHeatmap :trackId="trackComponent.key" :sessionId="$props.sessionId">
            <template #leftSpace>
              <div style="width: 100px; min-height: 60px">
              </div>
            </template>
            <template #trackTitle>
              <h5 v-show="!isOutside" class="text absolute top-1 left-25" size="small">{{ trackComponent.trackConfig.name
              }}</h5>
            </template>
            <template #canvas>
        ...
        <div class="flex mt-8">
          ...
          <div class="flex  aspect-square items-center justify-center w-full">
            <label for="dropzone-file"
              ...
              <div class="flex flex-col items-center justify-center pt-5 pb-6">
                ...
                <p class="mb-2 text-xl text-gray-500 dark:text-gray-400"><span class="font-semibold">Click to load heatmap
                    track</span></p>
              </div>
        ...
        <!-- The last track is the heatmap -->
        <!-- <track-item-square-heatmap :trackId="HeatmapComponent.key" :sessionId="$props.sessionId" :type="squareHeatmap">
          <template #chromViewV>
            <chrom-view-v :chrom="corenavStore.chrom" :chromBands="chromBands" :chromNames="chromNames"
              :chromSizes="chromSizes" :end="corenavStore.end" :isSelectorVisible="true" :labelWidth="0"
              :max="corenavStore.max" :min="corenavStore.min" :start="corenavStore.start" type="full" @zoomTo="zoomTo">
          ...
          <template #trackTitle>
            <h5 v-show="!isOutside" class="text absolute top-1 left-1" size="small">{{
              HeatmapComponent.trackConfig.name
            }}</h5>
          </template>
          <template #canvas>
            <component :is="HeatmapComponent.trackConfig.chartKey" :key="index" :config="HeatmapComponent"
              :dataLoaded="isLoaded" :location="corenavStore" :sessionId="sessionId" :style="{
                ...useHeightStyle(HeatmapComponent.attr)
              }" :trackId="HeatmapComponent.key" @zoomTo="zoomTo"></component>
          </template>
          <template #controller>
            <component :is="HeatmapComponent.trackConfig.controllerKey" v-if="HeatmapComponent.trackConfig.controllerKey"
              :key="index" :controllerConfig="HeatmapComponent.controllerConfig" :location="corenavStore">
            </component>
          </template>
