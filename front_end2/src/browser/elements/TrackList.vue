<template>
  <div v-if="isLoaded" :id="props.sessionId" class="track-list flex flex-col pl-2 pr-2 w-full">
    ...
    <chrom-view-h :chrom="corenavStore.chrom" :chromBands="chromBands" :chromNames="chromNames" :chromSizes="chromSizes"
      ...
      :start="corenavStore.start" type="full" @zoomTo="zoomTo">
    </chrom-view-h>
    ...
    <template v-for="(trackComponent, index) in trackComponents">
      <track-item :trackId="trackComponent.key" :sessionId="$props.sessionId">
        <template #trackTitle>
          <h5 v-show="!isOutside" class="text absolute top-1 left-1" size="small" >{{ trackComponent.trackConfig.name }}</h5>
        </template>
        <template #canvas>
...
const trackComponents = computed(() => {
  ...
  console.log('hhhhhhh',props.session)
