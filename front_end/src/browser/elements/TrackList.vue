...
const zoomTo2 = (chrom, start, end) => {
  ...
  if (end - start >= minRegionSize) {
    console.log('corenavStore.value.chrom = chrom', chrom)
...
const handleMenuSelect = (action) => {
  ...
  //   modalStore.openModal(response.data)
  // } catch (error) {
  //   // 错误处理
  //   console.error(error)
  // }
...
const setupVerticalLine = () => {
  console.log('setupVerticalLine')
  console.log(isShowTrackLabel.value)
...
watch(sessionTracks, (newTracks, oldTracks) => {
  if (newTracks.length > oldTracks.length) {
    ...
    console.log(sessionTracks.value)
    console.log(trackComponents.value)
  }
}, {immediate: false})
