...
<n-card class="browser-container relative" >
    <TrackList :assembly="props.assembly" :session="newSession" :sessionId="newSession.key" :tracks="newSession.trackIds">
    </TrackList>
</n-card>
</div>
...
const sessionStore = useSessionStore();
    //  const trackStore = useTrackStore(pinia);
...
const AddTracksToSession = (tracksInfo, existingTrackIds = []) => {
    ...
    const newTracks = tracksInfo.filter(track => 
...
// Add this computed property to get current track IDs
const currentTrackIds = computed(() => 
  newSession.value.trackIds || []
)

// Watch tracksInfo for changes
watch(() => props.tracksInfo, (newTracks, oldTracks) => {
  if (!newTracks) return;
...
// watch(()=>props.tracksInfo,(newValue)=>{
//   AddTracksToSession(newValue)
// },{immediate: true})

// onMounted(() => {
//   AddTracksToSession(props.tracksInfo)
// })
</script>
<style scoped>
</style>