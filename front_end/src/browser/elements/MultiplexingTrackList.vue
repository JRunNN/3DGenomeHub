<template>
    <div v-if="isLoaded" :id="props.sessionId" class="track-list flex flex-col pl-2 pr-2 w-full">
      <!-- <nav-bar v-model:chrom="corenavStore.chrom" :asm="$props.assembly.label" :chromNames="chromNames" :chromSizes="chromSizes"
      ...
      <!-- <div class="track-overlay absolute left-0 top-0" id="add-track-container" ref="LineContainer"
      :style="`width: ${width - 32}px; height: ${height}px`"
      @mousedown="onMouseDown"
      @mousemove="onMouseMove"
      @mouseup="onMouseUp"
      >
      <canvas 
      ref="canvasRef" :width="`${width - 32}`" :height="`${height}`"></canvas>
    </div> -->
    </div>
  </div>
    </div>
  ...
  import TrackItem from '@/browser/elements/MultiplexingTrackItem.vue'
  import { useHeightStyle } from "@/browser_new/hooks/useStyle";
  import Sortable from 'sortablejs';
  import { useMouseInElement } from '@vueuse/core'
  import { useTrackStore } from '@/browser_new/store'
  import { useSessionStore } from '@/browser_new/store'
